// Adapted for Youmu from Snapzy's scrolling-capture stitcher.
// Copyright (c) 2026, Trong Duong Duc.
// Licensed under the BSD 3-Clause License; see
// IntegratedFeatures/YoumuFeature/THIRD_PARTY_NOTICES.md.
// Upstream commit: 0405020e23598fa23aa5d530af3570aac2d3b292.
//
...
nonisolated final class ScrollingCaptureStitcher: @unchecked Sendable {
  ...
  private struct ContentSlice {
    ...
    let rowCount: Int
    let bytesPerRow: Int
    let sourceDocumentOffset: Int
    let sourceStartRow: Int
    var pixels: [UInt8]

    init?(
      copying raster: RasterImage,
      startRow: Int,
      rowCount: Int,
      role: Role,
      sourceDocumentOffset: Int
    ) {
      guard startRow >= 0, rowCount > 0, startRow + rowCount <= raster.height else {
        return nil
      }

      let byteStart = startRow * raster.bytesPerRow
      let byteEnd = (startRow + rowCount) * raster.bytesPerRow
      var ownedPixels = [UInt8](repeating: 0, count: byteEnd - byteStart)
      ownedPixels.withUnsafeMutableBytes { destination in
        raster.pixels.withUnsafeBytes { source in
          ...
          destinationBase.copyMemory(
            from: sourceBase.advanced(by: byteStart),
      ...
      self.bytesPerRow = raster.bytesPerRow
      self.sourceDocumentOffset = sourceDocumentOffset
      self.sourceStartRow = startRow
      // Keep an independently allocated, tightly packed row buffer. An ArraySlice here would
      // retain the complete source frame's backing allocation.
      self.pixels = ownedPixels
    }

    var sourceRows: Range<Int> {
      sourceStartRow..<(sourceStartRow + rowCount)
    }

    func copyRows(into destination: inout [UInt8], destinationRow: Int) {
      let destinationStart = destinationRow * bytesPerRow
      destination[destinationStart..<(destinationStart + pixels.count)] = pixels[...]
    }

    func copyRows(into destination: UnsafeMutableRawPointer, destinationRow: Int) {
      pixels.withUnsafeBytes { source in
        guard let sourceBase = source.baseAddress else { return }
        destination.advanced(by: destinationRow * bytesPerRow)
          .copyMemory(from: sourceBase, byteCount: pixels.count)
      }
    }

    func makeCGImage() -> CGImage? {
      RasterImage.makeCGImage(
        width: width,
        height: rowCount,
        bytesPerRow: bytesPerRow,
        pixels: pixels
  ...
  private struct VisionAlignmentEstimate {
    /// Positive means that the viewport moved down through the document and new rows belong at
    /// the bottom. Negative means the user moved back up and must never extend the output.
    let deltaY: Int
  ...
  /// Bytes retained by the accepted output timeline itself. This excludes the two full-frame
  /// matching rasters and any lazily rendered CGImage cache.
  var retainedPixelBytes: Int {
    contentSlices.reduce(0) { $0 + $1.pixels.count }
  }

  func start(
    with image: CGImage,
    maxOutputHeight: Int = Int.max
  ) -> ScrollingCaptureStitchUpdate? {
    guard let raster = RasterImage(cgImage: image) else { return nil }
    let acceptedHeight = min(
      raster.height,
      maxOutputHeight,
      ScrollCapturePolicy.maximumOutputHeight(pixelWidth: raster.width))
    guard acceptedHeight > 0 else { return nil }

    baseRaster = raster
    lastRaster = raster
    guard
      let initialSlice = ContentSlice(
        copying: raster,
        startRow: 0,
        rowCount: acceptedHeight,
        role: .scrollingContent,
        sourceDocumentOffset: 0
      )
    else {
      return nil
    }
    contentSlices = [initialSlice]
    headerHeight = 0
    footerHeight = 0
    leadingStaticWidth = 0
    trailingStaticWidth = 0
    mergeDirection = .unresolved
    cachedMergedImage = acceptedHeight == raster.height ? image : nil
    lastMatch = nil
    ...
    return ScrollingCaptureStitchUpdate(
      outcome: .initialized,
      mergedImage: acceptedHeight == raster.height ? image : mergedImage(),
  ...
  func append(
    ...
    if let expectedSignedDeltaPixels, expectedSignedDeltaPixels < 0 {
    ...
    if
      ...
      hasReliableOppositeVisionDirection(visionAlignmentEstimate)
    {
      return currentUpdate(
        outcome: .ignoredNoMovement,
        includeMergedImage: renderMergedImage,
    ...
    guard match.direction == .appendFromBottom else {
      return currentUpdate(
        outcome: .ignoredNoMovement,
        includeMergedImage: renderMergedImage,
        likelyReachedBoundary: false,
    ...
    if mergeDirection == .unresolved {
      ...
      leadingStaticWidth = inferredLeadingStaticWidth
      trailingStaticWidth = inferredTrailingStaticWidth
      bootstrapContentSlices(with: baseRaster, maximumHeight: effectiveHeightLimit)
      // Once the first movement has split the initial frame into owned slices, keeping the full
      // initial raster only duplicates a viewport-sized allocation for the rest of the session.
      self.baseRaster = nil
    }

    let remainingHeight = effectiveHeightLimit - outputHeight
    ...
    let currentFrameDocumentOffset = currentDocumentOffset + match.deltaY
    guard let sliceStart = sliceStartRow(for: match.direction, in: raster, deltaY: match.deltaY) else {
      matchNotFoundCount += 1
      return currentUpdate(
        outcome: .ignoredAlignmentFailed,
        includeMergedImage: renderMergedImage,
    ...
    let acceptedSlice = ContentSlice(
      copying: raster,
      startRow: sliceStart,
      rowCount: acceptedDelta,
      role: .scrollingContent,
      sourceDocumentOffset: currentFrameDocumentOffset
    )
    let replacementFooter = footerHeight > 0
      ? ContentSlice(
        copying: raster,
        startRow: raster.height - footerHeight,
    ...
    guard let acceptedSlice, footerHeight == 0 || replacementFooter != nil else {
      matchNotFoundCount += 1
      return currentUpdate(
        outcome: .ignoredAlignmentFailed,
        includeMergedImage: renderMergedImage,
    ...
    // Re-observe every already accepted document row that is visible in this frame before
    // extending the tail. A viewport-fixed control is thereby replaced at its old document
    // coordinate by the later frame's real page pixels and can exist only at the newest tail.
    // When the control disappears at the page boundary, the clean final frame removes it fully.
    refreshOverlappingContent(from: raster, documentOffset: currentFrameDocumentOffset)

    if
      let footerIndex = contentSlices.firstIndex(where: { $0.role == .fixedFooter }),
      let replacementFooter
    {
      contentSlices.insert(acceptedSlice, at: footerIndex)
      contentSlices[footerIndex + 1] = replacementFooter
    } else {
      contentSlices.append(acceptedSlice)
    }
    currentDocumentOffset = currentFrameDocumentOffset
  ...
  func mergedImage() -> CGImage? {
    ...
    guard let firstSlice = contentSlices.first else { return nil }
    let width = firstSlice.width
    let bytesPerRow = firstSlice.bytesPerRow
    let height = outputHeight
    guard height > 0,
      height <= ScrollCapturePolicy.maximumOutputHeight(pixelWidth: width),
      let context = CGContext(
        data: nil,
        width: width,
        height: height,
    ...
    for slice in contentSlices {
      slice.copyRows(into: destination, destinationRow: destinationRow)
      destinationRow += slice.rowCount
    }

    cachedMergedImage = context.makeImage()
    return cachedMergedImage
  }
  ...
  func previewImage(maxPixelWidth: Int, maxPixelHeight: Int) -> CGImage? {
    ...
    let targetScale = min(
      1,
      Double(safeMaxPixelWidth) / Double(firstSlice.width),
      Double(safeMaxPixelHeight) / Double(max(outputHeight, 1))
    )

    let targetWidth = max(1, Int((Double(firstSlice.width) * targetScale).rounded()))
    ...
    var destinationRow = 0
    for slice in contentSlices {
      guard let sliceImage = slice.makeCGImage() else {
        destinationRow += slice.rowCount
        continue
      }
  ...
  private func bootstrapContentSlices(
    with baseRaster: RasterImage,
    maximumHeight: Int
  ) {
    let retainedHeight = min(baseRaster.height, max(0, maximumHeight))
    guard retainedHeight > 0 else {
      contentSlices = []
      return
    }
    let retainedHeaderHeight = min(headerHeight, retainedHeight)
    let retainedFooterHeight = min(footerHeight, retainedHeight - retainedHeaderHeight)
    let contentStart = retainedHeaderHeight
    let contentHeight = max(0, retainedHeight - retainedHeaderHeight - retainedFooterHeight)
    var bootstrapped: [ContentSlice] = []

    if
      retainedHeaderHeight > 0,
      let header = ContentSlice(
        copying: baseRaster,
        startRow: 0,
        rowCount: retainedHeaderHeight,
        role: .fixedHeader,
        sourceDocumentOffset: 0
      )
    {
      bootstrapped.append(header)
    }
    ...
    {
      bootstrapped.append(content)
    }

    if
      retainedFooterHeight > 0,
      let footer = ContentSlice(
        copying: baseRaster,
        startRow: baseRaster.height - retainedFooterHeight,
        rowCount: retainedFooterHeight,
        role: .fixedFooter,
        sourceDocumentOffset: 0
      )
    {
      bootstrapped.append(footer)
    }

    if !bootstrapped.isEmpty {
      contentSlices = bootstrapped
    }
  }
  ...
  private func sliceStartRow(
    ...
    case .appendFromTop:
      // Upward motion is a navigation correction, not new document content. Supporting a top
      // append here would silently reverse the output timeline and poison future alignment.
      return nil
    case .unresolved:
      return nil
    }
  ...
  private func refreshOverlappingContent(
    from raster: RasterImage,
    documentOffset: Int
  ) {
    let visibleDocumentRange =
      (documentOffset + headerHeight)..<(documentOffset + raster.height - footerHeight)
    guard !visibleDocumentRange.isEmpty else { return }

    var refreshedPixels = false
    for sliceIndex in contentSlices.indices {
      guard contentSlices[sliceIndex].role == .scrollingContent else { continue }
      let sliceDocumentStart =
        contentSlices[sliceIndex].sourceDocumentOffset + contentSlices[sliceIndex].sourceStartRow
      let sliceDocumentRange =
        sliceDocumentStart..<(sliceDocumentStart + contentSlices[sliceIndex].rowCount)
      let refreshStart = max(sliceDocumentRange.lowerBound, visibleDocumentRange.lowerBound)
      let refreshEnd = min(sliceDocumentRange.upperBound, visibleDocumentRange.upperBound)
      guard refreshStart < refreshEnd else { continue }

      let sourceStartRow = refreshStart - documentOffset
      let destinationStartRow = refreshStart - sliceDocumentStart
      let rowCount = refreshEnd - refreshStart
      for rowOffset in 0..<rowCount {
        let sourceByteStart = (sourceStartRow + rowOffset) * raster.bytesPerRow
        let destinationByteStart =
          (destinationStartRow + rowOffset) * contentSlices[sliceIndex].bytesPerRow
        contentSlices[sliceIndex].pixels[
          destinationByteStart..<(destinationByteStart + raster.bytesPerRow)
        ] = raster.pixels[sourceByteStart..<(sourceByteStart + raster.bytesPerRow)]
      }
      refreshedPixels = true
    }
  ...
  private func bestMatch(
    ...
    guard mergeDirection != .appendFromTop else { return nil }
    let directions: [ScrollingCaptureMergeDirection] = [.appendFromBottom]
  ...
  private func hasReliableOppositeVisionDirection(
    _ visionAlignmentEstimate: VisionAlignmentEstimate?
  ) -> Bool {
    guard let visionAlignmentEstimate else { return false }
    guard visionAlignmentEstimate.deltaY <= -10 else { return false }
    guard visionAlignmentEstimate.agreementCount >= 2 else { return false }
    guard visionAlignmentEstimate.agreementCount * 2 >= visionAlignmentEstimate.observedCount else {
      return false
    }
    return visionAlignmentEstimate.deltaSpread <= max(10, abs(visionAlignmentEstimate.deltaY) / 5)
  }
  ...
  private func estimateVisionAlignment(
    ...
    for sample in sortedSamples {
      let tolerance = max(6, abs(sample) / 10)
      let cluster = sortedSamples.filter {
        ($0 < 0) == (sample < 0) && abs($0 - sample) <= tolerance
      }
  ...
  private func estimateVisionTranslation(
    ...
    // Vision returns the transform that aligns the current (targeted) crop to the previous crop.
    // A normal downward document scroll therefore has a negative transform.ty. Normalize that
    // into the stitcher's public convention: positive means append new rows at the bottom.
    ...
    let deltaY = Int(round(signedVerticalShift))
    let maxUsefulDelta = max(18, rowCount - max(72, Int(Double(rowCount) * 0.12)))
    guard abs(deltaY) <= maxUsefulDelta else { return nil }
